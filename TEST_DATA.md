...
## How to answer “13 is what?”

1. `TEST_REPORT.md` metric `runtime_contract_pass_rate`
2. `artifacts/latest/metrics.json` → `cases` array (13 names)
3. `artifacts/catalog.json` → `runtime_contracts`
4. `artifacts/latest/raw/contracts/<TestName>.jsonl` → `Action: pass`

Live 0/4: `artifacts/latest/raw/showcase.json` `runs[].status` and `runs[].run_id`.
