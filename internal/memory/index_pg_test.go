...
func TestSyncReconcilesEventsAndOrphans(t *testing.T) {
	ctx := context.Background()
	sqlDB := testdb.Open(t)
	sessionID := testdb.InsertSession(t, sqlDB)
	runID := ids.New()
	if _, err := sqlDB.ExecContext(ctx,
		`INSERT INTO runs (id,session_id,status,workspace_dir) VALUES ($1,$2,'completed','')`,
		runID, sessionID,
	); err != nil {
		t.Fatal(err)
	}
	...
	testdb.CleanupMemory(t, sqlDB, "orphan-sync-test")
	if _, err := sqlDB.ExecContext(ctx, `
...
func TestSearchRerankChangesOrder(t *testing.T) {
	...
	testdb.CleanupMemory(t, sqlDB, runA, runB)
	idx := New(sqlDB)
...
func TestRebuildRestoresDocsWithoutChangingEvents(t *testing.T) {
	...
	var nEvents int
	if err := sqlDB.QueryRowContext(ctx, `SELECT COUNT(*) FROM events WHERE run_id=$1`, runID).Scan(&nEvents); err != nil {
		t.Fatal(err)
	}
	var payload []byte
	if err := sqlDB.QueryRowContext(ctx,
	...
	var after int
	if err := sqlDB.QueryRowContext(ctx, `SELECT COUNT(*) FROM events WHERE run_id=$1`, runID).Scan(&after); err != nil {
		t.Fatal(err)
	}
	if after != nEvents {
